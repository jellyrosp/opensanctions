...
# print(gender_source_frequency(input_file))
...
#     """
#     Analyze the frequency of gender sources in the statements dataset,
#     including rows with empty or null gender values.
#     """
#     # Define the chunk size (optimized for 16GB RAM)
#     chunk_size = 50000
...
#     total_topics_rows = 0
#     total_gender_rows = 0
#     total_no_gender_rows = 0

#     # Sets to store unique canonical_id values
#     unique_topic_ids = set()
#     unique_gender_ids = set()
#     unique_no_gender_ids = set()
...
#         parse_dates=['first_seen', 'last_seen']
#     ):
#         # Filter rows for gender, topics, and missing gender values
#         gender_chunk = chunk[(chunk['schema'] == 'Person') & (chunk['prop'] == 'gender')]
#         topics_chunk = chunk[(chunk['schema'] == 'Person') & (chunk['prop'] == 'topics') & (chunk['value'] == 'sanction')]

#         # Filter rows where 'value' is null or empty for 'prop'="gender"
#         # no_gender_chunk = chunk[
#         #     (chunk['schema'] == 'Person') &
#         #     (chunk['prop'] == 'gender') &
#         #     (chunk['value'].isna() | (chunk['value'] == ""))
#         # ]
...
#         total_no_gender_rows += len(no_gender_chunk)

#         # Add unique canonical_id values to the respective sets
#         unique_topic_ids.update(topics_chunk['canonical_id'].unique())
#         unique_gender_ids.update(gender_chunk['canonical_id'].unique())
#         unique_no_gender_ids.update(no_gender_chunk['canonical_id'].unique())

#     # Print results
#     print(f"Total unique canonical_id count in topics_chunk with value 'sanction': {len(unique_topic_ids)}")
#     print(f"Total unique canonical_id count in gender_chunk: {len(unique_gender_ids)}")
#     print(f"Total unique canonical_id count with empty or null gender value: {len(unique_no_gender_ids)}")
#     print(f"Total rows with empty or null gender value: {total_no_gender_rows}")
