...
# def json_categorical_frequency_for_sanctioned_gender(category='nationality', min_count=50):
#     """
...
#             rows.append({'gender': gender, category: category_value, 'count': count})

#     df = pd.DataFrame(rows)

#     if df.empty:
#         print("No data found for the specified category and gender.")
#         return None

#     # Map category codes to full names if a mapping dictionary is provided
#     if country_mapping is not None:
#         df[f'{category}_name'] = df[category].map(country_mapping)
...
#     if country_mapping is not None:
#         filtered_df.loc[:, category_name_col] = filtered_df[category].map(country_mapping)
...
#     Returns:
#     - Dictionary with year as key and (relative_frequency_table, plot) as value
#     """
#     results = {}
...
#                     'plot': plt.gcf()  # Store the current figure
#                 }
